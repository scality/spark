...
# inner_join_false =  dfringkeys.join(dfs3keys,["digkey"], "leftanti").withColumn("is_present", F.lit(int(0))).select("ringkey", "is_present", "digkey", "objectkey")
df_final = inner_join_false.select("ringkey")
# df_final = inner_join_false.select("ringkey", "objectkey")
allmissing = "%s://%s/output/s3fsck/s3objects-missing-ring-%s.csv" % (PROT, PATH, RING)
allmissing2 = "%s://%s/%s/s3fsck/s3objects-missing.csv" % (PROT, PATH, RING)
df_final.write.format("csv").mode("overwrite").options(header="false").save(allmissing)
df_final.write.format("csv").mode("overwrite").options(header="false").save(allmissing2)