...
df_p0 = spark.read.format("csv").option("header", "false").option("inferSchema", "true").load(digkeys)

df_p2 = spark.read.format("csv").option("header", "false").option("inferSchema", "true").load(orphans)

df_p2_renamed = df_p2.withColumnRenamed("_c0", "_c0_renamed")

# df_final = df1.join(df2_renamed, df1['_c0'] == df2_renamed['_c0_renamed'], "leftanti").select(df1['_c0'])
df_final = df_p2_renamed.join(df_p0, df_p2_renamed['_c0_renamed'].rlike(df_p0['_c0'])).select(df_p2_renamed['_c0_renamed'])
# df_final = df1.join(df2, array_contains(split(df2("_c0_renamed"), "left"), df1("_c0")))
